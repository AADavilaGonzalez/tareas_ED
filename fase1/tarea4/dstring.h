...
static Dstring* dstring_crear(const char* str, size_t capacidad) {
    ...
    string->_c_str[string->_longitud]='\0';
    return string;
}
...
static void dstring_append_char(Dstring* str, char c) {
    if(str->_longitud > str->_capacidad)
        dstring_resize(str, DSTRING_PC(str->_capacidad));
    str->_c_str[str->_longitud]=c;
    str->_c_str[str->_longitud+1]='\0';
    ++(str->_longitud);
    return;
}
...
static int dstring_fset(Dstring* str, FILE* entrada, char del) {
    int c, i=0;
    while(isspace(c=fgetc(entrada)) && c!='\n');
    while(c!=del && c!=EOF) {
        if(i >= str->_capacidad)
            dstring_resize(str, DSTRING_PC(str->_capacidad));
        str->_c_str[i]=c;
