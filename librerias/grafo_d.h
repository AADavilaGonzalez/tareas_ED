...
static Grafo_D* grafo_d_crear(void) {
    Grafo_D* grafo = malloc(sizeof(Grafo_D));
...
static Vect_V* grafo_d_get_vertices(Grafo_D* grafo) {
    Vect_V* vec = malloc(sizeof(Vect_V)+sizeof(Vertice*)*grafo->orden);
    if(!vec) return NULL;
...
static Vertice* grafo_d_insertar_vertice(Grafo_D* grafo, Vertice vt) {
    Nodo_V* nuevo = malloc(sizeof(Nodo_V));
...
static const Vect_V* grafo_d_buscar_vertices(const Grafo_D* grafo, Vertice ref) {
    assert(grafo->cmp_vt!=NULL);
    Vect_V* vectmp = malloc(sizeof(Vect_V)+sizeof(Vertice*)*(grafo->orden));
    ...
    Vect_V* vector = malloc(sizeof(Vect_V)+sizeof(Vertice*)*(vectmp->tamano));
...
static Vect_A* grafo_d_get_aristas(Grafo_D* grafo) {
    Vect_A* vec = malloc(sizeof(Vect_A)+sizeof(Arista*)*grafo->tamano);
    if(!vec) return NULL;
...
static Arista* grafo_d_insertar_arista(Grafo_D* grafo, Arista ar, const Vertice* ini, const Vertice* fin) {
    Nodo_A* nuevo = malloc(sizeof(Nodo_A));
    if(!nuevo) return NULL;
...
static Vect_A* grafo_d_insertar_aristas_conjugadas(Grafo_D* grafo, Arista ar, const Vertice* ini, const Vertice* fin) {
    Nodo_A* ini_fin = malloc(sizeof(Nodo_A));
    if(!ini_fin) return NULL;
    Nodo_A* fin_ini = malloc(sizeof(Nodo_A));
    if(!fin_ini) {free(ini_fin); return NULL;}
    Vect_A* vect = malloc(sizeof(Vect_A)+sizeof(Arista*)*2);
...
static const Vect_A* grafo_d_buscar_aristas(const Grafo_D* grafo, Arista ref) {
    assert(grafo->cmp_ar!=NULL);
    Vect_A* vectmp = malloc(sizeof(Vect_A)+sizeof(Arista*)*(grafo->tamano));
    if(!vectmp) return NULL;
    vectmp->tamano=0;
    Nodo_V* vptr = grafo->lista_ady;
    while(vptr!=NULL) {
    ...
    Vect_A* vector = malloc(sizeof(Vect_A)+sizeof(Arista*)*(vectmp->tamano));
    if(!vector) {free(vectmp); return NULL;}
...
static const Vect_A* grafo_d_buscar_aristas_entre_vert(const Grafo_D* grafo, const Vertice* ini, const Vertice* fin) {
    ...
    Vect_A* vectmp = malloc(sizeof(Vect_A)+sizeof(Arista*)*vptr->grado_s);
    if(!vectmp) return NULL;
    vectmp->tamano=0;
    Nodo_A* aptr=vptr->lista_ady;
    while(aptr!=NULL) {
...
static MatrizAdy_D* grafo_d_generar_matriz_datos(const Grafo_D* grafo) {
    MatrizAdy_D* matriz = malloc(sizeof(MatrizAdy_D)+
...
static MatrizAdy_N* grafo_d_generar_matriz_ady(const Grafo_D* grafo) {
    MatrizAdy_N* matriz = malloc(sizeof(MatrizAdy_N)+
...
static MatrizAdy_P* grafo_d_generar_matriz_pesos(const Grafo_D* grafo) {
    MatrizAdy_P* matriz = malloc(sizeof(MatrizAdy_P)+
