/*Grafo_D V0.3: Libraria de TDA Implementado Mediante Lista de Adyacencia*/
#ifndef _GRAFO_D_H
#define _GRAFO_D_H
...
/*  LIBRERIA DE TDA DE GRAFO DINAMICO GRAFO_D
    Esta es una libreria de tipo cabezera que implementa el tipo de dato abstracto Grafo_D mediante
    una lista de adyacencia. Su alcance es limitado ya que las operaciones no estan optimizadas
    asimtoticamente debido este TDA fue imlementado unicamente con listas enlazadas.

    La libreria esta disenada para que tanto los vertices como las aristas puedan aceptar un valor 
    de tipo dato generico para su representacion asi como structs si se requiere de una 
    representacion mas compleja. Si no se especifica los tipos de dato de los vertices/aristas se
    asume que se representaran por medio de un tipo de dato (int).

    Las macros siguientes permiten la customizacion de los tipos de datos que se usaran para la
    representar los distintos componentes/propiedades del grafo. Deben definirse antes de incluir
    la libreria.
    ----------------------------------------------------------------------------------------

    DATO_VERTICE/STRUCT_VERTICE: Permite asignar el tipo de dato que se usara para representar a
    cada vertice de los grafos creados con la libreria. Solo uno de estos dos macros debe ser
    definido a la vez: DATO_VERTICE si el tipo definido es un tipo por default del lenguaje 
    (int,float,double,long, etc.) o STRUCT_VERTICE si el dato es un tipo de dato struct.
    Ejemplos:
    #define DATO_VERTICE double
    #define STRUCT_VERTICE Punto, con punto un struct definido por el usuario

    DATO_VERTICE/STRUCT_VERTICE: Permite asignar el tipo de dato que se usara para representar a
    cada arista de los grafos creados con la libreria. Solo uno de estos dos macros debe ser
    definido a la vez: DATO_ARISTA si el tipo definido es un tipo por default del lenguaje
    (int,float,double,long, etc.) o STRUCT_ARISTA si el dato es un tipo de dato struct.
    Ejemplos:
    #define DATO_ARISTA double
    #define STRUCT_ARISTA Punto, sea punto un struct definido por el usuario

    DATO_PESO: Permite asignar el tipo de dato que se usa para representar el peso de una arista,
    la matriz de pesos sera una matriz de este tipo de dato. Si se define un DATO_PESO antes de
    incluir la libreria, se debera definir tambien la macro PESO_NO_ARISTA. El valor de DATO_PESO
    tiene que ser un tipo por default numerico(float, int, etc). Ejemplo:
    ...
    entre dos vertices. Suele ser un valor muy alto en relacion a el tipo especificado.
    Ejemplo:
...
/*------------------------------Macros-de-Customizacion-de-la-Libreria------------------------------*/
...
/*  Crea un vector estatico en el heap capaz de contener hasta 'tamano' verices No es posible
    modificar el tamano del vector despues de crearlo                                               */
...
/*  Libera la memoria reservada para almacenar el vector. El puntero pasado a esta funcion queda
    invalidado despues de esta operacion                                                            */
static inline void vect_v_destruir(Vect_V* vect) {free(vect);}
...
/*  Libera la memoria reservada para almacenar el vector. El puntero pasado a esta funcion queda 
    invalidado despues de esta operacion                                                            */
static inline void vect_a_destruir(Vect_A* vect) {free(vect);}
...
/*  Crear un grafo vacio para empezar a realizar operaciones. Algunas funciones requieren que el
    grafo tenga asociado un comportamineto en particular, mediante las funciones
...
/*  Asigna una funcion bool cmp_vt(Vertice*, Vertice*) implementada por el usario de la libreria
    responsable de comparar las vertices del grafo. Esta funcion se debe registrarse
    obligatorimente cuando el usario ha definido los vertices del grafo como tipo struct y se va
    hacer uso de la funcion grafo_d_buscar_vertice().                                               */
...
static inline void grafo_d_unset_cmp_vt(Grafo_D* grafo) {grafo->cmp_vt=cmp_vt_default;}


/*  Asigna una funcion bool cmp_ar(Arista*, Arista*) implementada por el usuario de la libreria
    responsable de comparar las aristas de el grafo. Esta funcion debe registrarse obligatoriamente
    cuando el usuario ha definido las aristas del grafo como tipo struct y se va hacer uso de
    alguna de las funciones grafo_d_buscar_arista(s)_...() a excepcion de las funciones 
    grafo_d_buscar_arista(s)_entre_vert()                                                           */
...
/*  Asigna una funcion peso_t calc_peso(Arista*) implementada por el usuario de la libreria
    reponsable de asignar un peso en base a la informacion de una arista del grafo. Esta funcion
    nunca es obligatoria de registrar, su comportamiento por default es regresar (peso_t)1 para
    toda arista elemento del grafo.                                                                 */
...
static inline void grafo_d_unset_calc_peso(Grafo_D* grafo) {grafo->calc_peso=calc_peso_default;}
...
/*  Libera la memoria reservada dinamicamente para el grafo, el puntero a el grafo pasado a la
    funcion queda invalidado despues de esta operacion                                              */
...
/*  Regresa un vector dinamcamente alojado que contiene todos los vertices del grafo. El vector
    debe ser liberado con una llamada a vector_v_destruir()                                         */
...
/*  Inserta un vertice con los datos que contenga el agumento vt, devuelve la direccion de memoria
    que identifica al vertice como elemento unico del grafo, las demas operaciones que esperan un
    argumento vertice toman este valor de retorno para identificar el vertice                       */
...
/*  Regresa la direccion de memoria (identificador) del primer vertice que haga que la funcion
    grafo->cmp_vt() regrese como verdadero                                                          */
...
/*  Regresa un vector almacenado dinamicamente que contine los vertices que hacen que funcion 
    grafo->cmp_vt() regrese verdadero. El vector esta reservado con memoria dinamica por lo que
    necesita ser liberado con una llamada a vect_v_destruir()                                       */
...
/*  Elimna un unico vertice cuyo identificador esta dado por el argumento vt. Las aristas que
    concetan con el vertice vt tambien son eliminadas. Esta funcion espera como argumento vt un
    puntero valido a un vertice del grafo, resultado de las funciones crear o buscar vertices. De
    no ser asi el comportamiento de la funcion es indefinido.                                       */
...
/*  Regresa un vector dinamicamente alojado que contine todas las aristas del grafo. El vector
    debe ser liberado con una correspondiente llamada a vect_a_destruir()                           */
...
/*  Inserta una arista entre el vertice ini y el vertice fin que va de ini a fin. Los datos de la
    arista seran los suministrados mediante el argumento ar. La funcion devuelve la direccion de
    memoria (identificador) de la arista como elemento unico del grafo.                             */
static Arista* grafo_d_insertar_arista(Grafo_D* grafo, Arista ar, const Vertice* ini,
    const Vertice* fin)
{
...
/*  Inseta un par de aristas conjugadas (una arista de ini a fin y otra de fin a ini) entre dos
    vertices. Los datos para ambas aristas estan dados por el paramentro ar. La funcion devuelve
    un vector alamacenado dinamicamente de tamano 2 que contine ambas aristas. Debido a que el
    vector que regresa la funcion es dinamicamente alojado en memoria debe ser liberado con una
    llamada vect_a_destruir()                                                                       */
static Vect_A* grafo_d_insertar_aristas_conjugadas(Grafo_D* grafo, Arista ar, const Vertice* ini,
    const Vertice* fin)
{
...
/*  Inserta un par de aristas conjugadas (una de ini a fin y otra de fin a ini) en el grafo ambos
    con la informacion suministrada al paramentro ar. La funcion regresa si la insersion se
...
/*  Regresa la direccion de memoria(identificador) del la primer arista que haga que la funcion
    grafo->cmp_ar() regrese verdadero.                                                              */
...
/*  Regresa un vector almacenado dinamicamente con las direcciones de memoria de todas las aristas
    en el grafo que hagan que la funcion grafo->cmp_ar() regrese verdader. El vector debera ser
    liberado mediante una llamada a vect_a_destruir                                                 */
...
static Arista* grafo_d_buscar_arista_entre_vert(const Grafo_D* grafo, const Vertice* ini,
    const Vertice* fin)
{
    struct _nodo_v* vptr = grafo->lista_ady;
...
/*  Regresa un vector alamacenado dinamicamente con todas las aristas del grafo que tengan como
    inicio al vertice ini y de fin al vertice fin. El vector debera ser liberado mediante una 
    llamada a la funcion vect_a_destruir()                                                          */
static Vect_A* grafo_d_buscar_aristas_entre_vert(const Grafo_D* grafo, const Vertice* ini,
    const Vertice* fin)
{
    struct _nodo_v* vptr=grafo->lista_ady;
...
/*  Regresa la primera arista que haga que la funcion grafo->cmp_ar() regrese verdadero asi como
    que tenga como inicio al vertice ini y de fin al vertice fin                                    */
static Arista* grafo_d_buscar_arista_estricto(const Grafo_D* grafo, Arista ref, const Vertice* ini,
    const Vertice* fin)
{
    assert(grafo->cmp_ar!=NULL);
    struct _nodo_v* vptr=grafo->lista_ady;
...
/*  Regresa un vector alamacenado dinamicamente con todas las aristas del grafo que hagan que la
    funcion grafo->cmp_ar() regrese verdadero asi como que tenga como inicio al vertice ini y de
    fin al vertice fin. El vector debera ser liberado mediante una llamada a la funcion
    vect_a_destruir()                                                                               */
static Vect_A* grafo_d_buscar_aristas_estricto(const Grafo_D* grafo, Arista ref, const Vertice* ini,
    const Vertice* fin)
{
    assert(grafo->cmp_ar!=NULL);
    struct _nodo_v* vptr=grafo->lista_ady;
...
/*  Elimina la arista del grafo pasada como agumento ar. El argumento ar debe ser el resultado de
    llamar a alguna funcion de crear o buscar arista. En caso contrario, el comportamiento de esta
    funcio es indefinido                                                                            */
...
/*  Devuelve el elemento de una matriz de adyacencia o de pesos que corresponde a la arista del
    grafo que tiene inicio en el vertice agrado en la posicion i y fin el vertice agregado en la
    posicion j.                                                                                     */
#define GD_MATRIZ_INDEX(matriz, i, j) (*(matriz->datos+i*matriz->orden+j))

/*  Libera la memoria reservada para almacenar la matriz                                            */
#define GD_MATRIZ_DESTRUIR(matriz) free(matriz)


/*  Regresa una matiz de adyacencia de (grafo->orden)x(grafo->orden) almacenada dinamicamente
    cuyos ejes estan ordenados de acuerdo a el orden en el que los vertices fueron insertados en
    el grafo. La matriz contine la direccion de memoria a un dato tipo Arista en cada celda de la
    matriz. Todo acceso a la matriz de hacerse mediante la MACRO GD_MATRIZ_INDEX(matriz, i, j). La
    memoria reservada para la matriz debe de ser liberada mediante la MACRO GD_MATRIZ_DESTRUIR().
    Esta matriz NO TOMA EN CUENTA varias aristas entre los mismos vertices (multigrafos), tomando
    por defecto la primera arista encontrada para formar la matriz                                  */
...
/*  Regresa una matiz de adyacencia de (grafo->orden)x(grafo->orden) almacenada dinamicamente
    cuyos ejes estan ordenados de acuerdo a el orden en el que los vertices fueron insertados en
    el grafo. La matriz almacena la cantidad de conexiones entre aristas de acuerdo al criterio de
    matriz de ayacencia definido matematicamente para grafos ponderados. Todo acceso a la matriz
    de hacerse mediante la MACRO GD_MATRIZ_INDEX(matriz, i, j). La memoria reservada para la
    matriz debe de ser liberada medianteuna la MACRO GD_MATRIZ_DESRUIR()                            */
...
/*  Regresa una matiz de adyacencia de (grafo->orden)x(grafo->orden) almacenada dinamicamente
    cuyos ejes estan ordenados de acuerdo a el orden en el que los vertices fueron insertados en
    el grafo. La matriz contine la direccion de memoria a un dato tipo peso_t en cada celda de la
    matriz. El valor de cada celda esta determinado por el resultado de la funcion
    grafo->calc_peso() sobre la arista correspondiente a cada posicion. Todo acceso a la matriz de
    hacerse mediante la MACRO GD_MATRIZ_INDEX(matriz, i, j). La memoria reservada para la matriz
    debe de ser liberada mediante la MACRO GD_MATRIZ_DESTRUIR. Esta matriz NO TOMA EN CUENTA
    varias aristas entre los mismos vertices (multigrafos), tomando por defecto la primera arista
    encontrada                                                                                      */
...
/*  !!!FUNCION DE USO INTERNO!!!
    Reserva memoria contigua para un Camino_D no nulo en vez de hacer varias reservaciones por
    separado para los vertices y aristas                                                            */
...
/*  Crea un camino nulo que representa la ausencia de un camino entre dos vertices. La memoria debe
    ser liberada manualmente mediante una llamada a camino_d_destruir()                             */
...
/*  Crea un camino trivial que solo contiene el vertice indicado. La memoria del
    debe ser liberada manualmente mediante una llamada a camino_d_destruir                          */
...
/*  Libera la memoria reservada en memoria para el camino                                           */
static inline void camino_d_destruir(Camino_D* camino) {free(camino);}
...
/*  Regresa el subcamino culla cantidad de saltos esta dada por 'indice' es decir regresa un camino
    conformado por los primeros 'indice'+1 vertices del camino original                             */
...
/*  Version de camino_d_recotar que regresa la version recortada de camino y se encarga de liberar
    la memoria utilizada por camino para hacer posible asignaciones como
    camino = camino_d_rctr(camino, i) sin filtrar memoria                                           */
static inline Camino_D* camino_d_rctr(Camino_D* camino, size_t indice) {
    Camino_D* tmp = camino_d_recortar(camino, indice); free(camino);
    return tmp; 
}


/*  Regreasa un nuevo camino conformado por los contenidos de c1 concatenados a los contenidos de
    c2. Para concatenar los caminos exitosamente, ambos no deben sernulos y c1 debe terminar en el
    mismo vertice en el que c2 empieza. De lo contrario, se devolvera un camino nulo.               */
...
/*  Version de camino_d_concatenar que la version concatenada de c1 y c2 y se encarga de liberar
    la memoria de ambos para hacer posibles asignaciones como
    camino = camino_d_concat(camino1, camino2) sin filtrar memoria                                  */
static inline Camino_D* camino_d_concat(Camino_D* c1, Camino_D* c2) {
...
/*  !!!FUNCION DE USO INTERNO!!!
    Busca en la lista de Nodos_C el Nodo_C que hace referencia a el struct _nodo_v pasado como
    segundo argumento.                                                                              */
...
/*  !!!FUNCION DE USO INTERNO!!!
    Busca en la lista de Nodos_C el Nodo_C con el valor de dist_orig minimo (y finito) en toda la
    lista.                                                                                          */
...
/*  !!!FUNCION DE USO INTERNO!!!
    Libera el espacio de memoria de la lista de Nodos_C apuntada por el puntero cptr. cptr debe
    ser la cabeza de la lista para evitar perdida de memoria                                        */
...
/*  !!!FUNCION DE USO INTERNO!!!!
    Construye un Camino_D en base a la infomacion recolectada durante la ejecucion del
    algorimo de dijsktra. Se asume que nodo_fin es el nodo correspondiente a un vertice que
    ya ha sido encontrado y que la lista que contine nodo_fin sigue siendo valida                   */
...
/*  Encuentra el camino mas corto entre el vertice ini y fin utilizando el algoritmo de Dijkstra.
    El camino se regresa como un struct tipo Camino_D dinamicamente almacenado en memoria y por lo
    tanto debera de liberarse utilizando la funcion camino_d_destruir(). En caso de error la funcion
    regresa NULL. En caso de que no exista ningun camino entre el los vertice ini y fin, la
    funcion regresa un camino invalido, no NULL, e igualmente debera ser liberado usando
    camino_d_destruir().                                                                            */
...
/*  Ordena los vertices en la lista de adyacencia del grafo de acuerdo a la funcion proporcionada
    como segundo argumento. La funcion orden debe regresar la posicion de el primer vertice con
    respecto al segundo en terminos de el enum orden_t (ANTES, DESPUES)                             */
...
/*  !!!FUNCION DE USO INTERNO!!!
    Ordena la lista de ayacencia del struct _nodo_v pasado como primer argumento de acuerdo a la
    funcion proporcionada como segundo argumento. La funcion orden debe regresar la posicion de el
...
/*  Ordena toda la represetacion interna del grafo de acuerdo a las dos funciones proporcionadas
    como argumentos. Cada funcion orden debe regresar la posicion de el primer vertice/arista con
    respecto al segundo en terminos de el enum orden_t (ANTES, DESPUES)                             */
...
/*  Imprime una representacion cruda del grafo en su forma de lista de adyacencia Los vertices y
    aristas del grafo estan identificados por su direccion de memoria el cual es unico para cada
    vertice/arista.                                                                                 */
...
static inline void grafo_d_print_debug(const Grafo_D* grafo) {grafo_d_fprint_debug(grafo, stdout);}


/*  Imprime la lista de adyacencia de el grafo pasado como primer argumeto. Se debe suministrar
    una funcion responsable de imprimir la representacion de cada vertice del grafo como segundo
    argumento. Es necesario que la funcion imprima los datos de cada vertice en una sola linea
    para garantizar un formateo correcto                                                            */
...
/*  Imprime la lista de adyacencia de el grafo ponderado pasado como primer argumeto. Se debe
    suministrar una funcion responsable de imprimir la representacide cada vertice del grafo como
    segundo argumento y otra funcion responsable de imprimir el peso de cada arista como tercer
    argumento. Es necesario que la funcion imprima los datos de los vertices y pesos  en una sola
    linea para garantizar un formateo correcto. El peso del grafo sera calculado a partir de la
    funcion suministrada mediante grafo_d_set_calc_peso()                                           */
